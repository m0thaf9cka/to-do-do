export const FILTER_ALL_VALUE = 'all';
export const FILTER_ACTIVE_VALUE = 'active';
export const FILTER_COMPLETE_VALUE = 'complete';

export const FILTER_ALL_LABEL = 'All';
export const FILTER_ACTIVE_LABEL = 'Active';
export const FILTER_COMPLETE_LABEL = 'Complete';

export const SORT_CREATED_AT_ASC = 'createdAt-asc';
export const SORT_CREATED_AT_DESC = 'createdAt-desc';
export const SORT_TITLE_ASC = 'title-asc';
export const SORT_TITLE_DESC = 'title-desc';

export const SEARCH_LABEL = 'Search';
